...
async def test_migrate_entry_from_1(_, mock_async_fetch_device_info, hass: HomeAssistant):
...
async def test_migrate_entry_from_2(mock_async_entries_for_config_entry, _, mock_async_fetch_device_info,
                                    hass: HomeAssistant):
...
@patch.object(WibeeeAPI, 'async_fetch_values', autospec=True)
@patch.object(er, 'async_entries_for_config_entry', autospec=True)
async def test_migrate_entry_from_2_offline(mock_async_entries_for_config_entry, _, __, hass: HomeAssistant):
    entry = MockConfigEntry(domain='wibeee', data={'host': '127.0.0.2'}, options={'scan_interval': 30, 'nest_upstream': 'proxy_disabled'},
                            version=2)
    mock_async_entries_for_config_entry.side_effect = lambda _, entry_id: [] if entry_id != entry.entry_id else [
...
@patch.object(WibeeeAPI, 'async_fetch_device_info', autospec=True)
@patch.object(WibeeeAPI, 'async_fetch_values', autospec=True)
async def test_migrate_entry_from_3(_, __, hass: HomeAssistant):
    entry = MockConfigEntry(domain='wibeee',
                            data={'host': '127.0.0.127', 'mac_address': 'abcdabcdabcd', 'wibeee_id': 'WIBEEE'},
                            options={'scan_interval': 123, 'nest_upstream': 'proxy_disabled'},
                            version=3)
...
@patch.object(WibeeeAPI, 'async_fetch_device_info', autospec=True)
@patch.object(WibeeeAPI, 'async_fetch_values', autospec=True)
async def test_migrate_entry_from_4(_, __, hass: HomeAssistant):
    entry = MockConfigEntry(domain='wibeee',
                            data={'host': '127.0.0.127', 'mac_address': 'abcdabcdabcd', 'wibeee_id': 'WIBEEE'},
