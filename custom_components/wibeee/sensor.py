...
KNOWN_SENSORS = [
    ...
    SensorType(
        "irms", "i", "Irms", "Current", ELECTRIC_CURRENT_AMPERE, DEVICE_CLASS_CURRENT
    ),
    SensorType(
        "frecuencia", "q", "Frequency", "Frequency", FREQUENCY_HERTZ, device_class=None
    ),
    SensorType(
        "p_activa", "a", "Active_Power", "Active Power", POWER_WATT, DEVICE_CLASS_POWER
    ),
    SensorType(
        "p_reactiva_ind",
        "r",
...
def create_sensors(device, status) -> list["WibeeeSensor"]:
    xml_suffixes = {
        sensor_type.status_xml_suffix: sensor_type for sensor_type in KNOWN_SENSORS
    }
    phase_xml_suffixes = [
        (key[4:].split("_", 1), value)
        for key, value in status.items()
        if key.startswith("fase")
    ]
    known_sensors = [
        (phase, xml_suffixes[suffix], value)
        for ((phase, suffix), value) in phase_xml_suffixes
        if suffix in xml_suffixes
    ]

    return [
        WibeeeSensor(device, phase, sensor_type, initial_value)
        for (phase, sensor_type, initial_value) in known_sensors
    ]
...
def update_sensors(sensors, update_source, lookup_key, data):
    ...
    _LOGGER.debug(
        "Received %d sensor values from %s: %s",
        len(sensors_with_updates),
        update_source,
        data,
        sensors_with_updates,
    )
...
async def async_setup_local_push(
    hass: HomeAssistant, device, sensors: list["WibeeeSensor"]
):
...
async def async_setup_entry(
    ...
    timeout = timedelta(
        seconds=entry.options.get(CONF_TIMEOUT, DEFAULT_TIMEOUT.total_seconds())
    )
    ...
    _LOGGER.info(
        f"Setup completed for '{entry.unique_id}' (host={host}, scan_interval={scan_interval}, timeout={timeout})"
    )
    return True
...
class WibeeeSensor(SensorEntity):
    ...
    def __init__(
        ...
        entity_id = slugify(
            f"{DOMAIN} {mac_addr} {sensor_type.friendly_name} L{sensor_phase}"
        )
        ...
        self._attr_state_class = (
            STATE_CLASS_TOTAL_INCREASING
            if sensor_type.device_class is DEVICE_CLASS_ENERGY
            else STATE_CLASS_MEASUREMENT
        )
        self._attr_device_class = sensor_type.device_class
        self._attr_unique_id = (
            f"_{mac_addr}_{sensor_type.unique_name.lower()}_{sensor_phase}"
        )
        ...
        self.entity_id = (
            f"sensor.{entity_id}"  # we don't want this derived from the name
        )
